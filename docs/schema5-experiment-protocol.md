...
## 11. Amendment 1: withdrawn, 2026-08-12

Proposed and withdrawn the same day, before any metric was computed against a relabelled row. It is
kept here rather than deleted because the measurement that refuted it constrains what a successor
protocol may assume. Evidence: `docs/schema5-screening-outcome.md`.

### What was proposed

Screen labels are `atan2(offset_mm, viewingDistanceMM)`, and every schema-5 session declared
`viewingDistanceMM = 550` — typed into Settings, never measured, because macOS reports no camera
field of view. Canonical crop side scales as `1/distance`, so the amendment proposed recovering a
per-session distance from crop scale, anchoring it with one physical measurement, and recomputing
every screen label.

### Why it was withdrawn

Crop side does not measure distance. It is a fixed multiple of the eye's own axis length, and that
length depends on head pose as strongly as it depends on distance:

- within-session correlation with head pitch is `r = 0.28` to `0.68`, slope about `0.4 px/deg`
- within-session correlation with `|yaw|` reaches `r = -0.69`, the foreshortening the crop contract
  already documents
- `lookDown` has the largest crop side in all six sessions, because the chin is down

The confound is not a correction term, it is comparable to the signal. Three defensible estimators —
median over all rows, median over the neutral block, and the neutral block corrected for pitch —
disagree by up to `72 mm` on the same session. Correcting a corner label to `0.2°` requires distance
to roughly `1%`, about `5 mm`. The method is about fourteen times too imprecise for its purpose.

An anchor was measured with a rule on 2026-08-12: `550 mm` at a simultaneous crop-side reading of
`128.4 px`, giving `k = 70,620 px·mm`. The anchor is sound and is recorded for reuse. It is not what
failed; the pose-confounded inference is.

### What this establishes for a successor protocol

1. The label defect is real and measured. The six sessions were not recorded at one distance, and
   one training session carries about `3.5°` of corner-label error against a `2°` median gate. The
   labels contradict each other, which is why no declared factor could satisfy them.
2. The defect cannot be repaired after recording. Viewing distance is not recoverable from the
   image at the precision labels require, and rule 7 of section 8 forbids a new recording under this
   protocol.
3. Therefore schema 5 closes without a pass, and without a repairable dataset.
4. A successor protocol must measure viewing distance at recording time rather than declaring it,
   and record it per session alongside the crop-scale reading so drift is detectable. The collector
   now shows canonical crop side live in the diagnostics HUD, which makes seating drift observable
   while recording rather than in an offline audit.
5. The physical-lens target remains the one label that is correct by construction at any distance.
   Any successor should keep it, and should treat lens-versus-screen error divergence as the first
   diagnostic for label geometry faults.
